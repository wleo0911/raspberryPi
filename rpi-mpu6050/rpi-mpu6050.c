// MPU6050 code.
// gpio readall           gpio 확인
// sudo i2cdetect -y 1    연결된 장치 확인
...
{
    int fd;
    char *fileName = "/dev/i2c-1"; // device drive를 접근하기 위해
    int  address = 0x68;
    ...
    if (ioctl(fd, I2C_SLAVE, address) < 0) { // ioctl 사용자 명령어
        printf("Unable to get bus access to talk to slave\n");
        exit(1);
    }
    
    int8_t power = i2c_smbus_read_byte_data(fd, MPU_POWER1);
    i2c_smbus_write_byte_data(fd, MPU_POWER1, ~(1 << 6) & power); //Wire.write(0)
